...
function makeDateStr( start = 0, end = 0 ) {
  ...
  if ( start ) {
    ...
    if ( end ) {
      ...
      // if the difference in the start and end time is greater than 24 hrs, show the ending date
      if ( ( timeDifference / ( 1000 * 60 * 60 ) ) > 24 ) {

        dateStr += ' - ';

        const endDay = new Date( startDate.getTime() + timeDifference );

        if ( endObj.month > startObj.month ) {
          dateStr += `${ months[ endObj.month ] }\xa0`;
        }

        dateStr += endDay.getDate();
      }
    }
  }
