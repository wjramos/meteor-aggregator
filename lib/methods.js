    ...
    if ( start ) {
      ...
      if ( end ) {
        ...
        if ( endObj.month !== startObj.month || endObj.date !== startObj.date ) {
          endStr = Meteor.call ( 'assembleDateStr', endStr, ' - ' );
        }

        if ( endObj.month !== startObj.month ) {
          endStr = Meteor.call ( 'assembleDateStr', endStr, endObj.month + ' ' );
        }

        if ( endObj.date !== startObj.date ) {
          endStr = Meteor.call ( 'assembleDateStr', endStr, endObj.date );
        }
      }
